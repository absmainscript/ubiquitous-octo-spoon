...
export function BasicInfoForm({ configs }: BasicInfoFormProps) {
  ...
  const basicSchema = z.object({
    ...
    aboutSectionName: z.string().min(1, "Nome da seção sobre é obrigatório"),
  ...
  const getBasicData = () => {
    ...
    return {
      ...
      aboutSectionName: generalInfo.aboutSectionName || generalInfo.name || "Dra. Adrielle Benhossi",
      ...
      const promises = [
        apiRequest("POST", "/api/admin/config", {
          ...
          value: {
            ...
            aboutSectionName: data.aboutSectionName,
            crp: data.crp,
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <FormField
              control={form.control}
              name="aboutSectionName"
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Nome na Seção "Sobre"</FormLabel>
                  <FormControl>
                    <Input placeholder="Dra. (Adrielle Benhossi)" {...field} />
                  </FormControl>
                  <FormDescription>
                    Nome que aparece no título da seção sobre (pode usar parênteses para gradiente)
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
